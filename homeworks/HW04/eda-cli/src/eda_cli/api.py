...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт, который принимает CSV-файл, вычисляет флаги качества через summarize_dataset / missing_table /
    compute_quality_flags и возвращает словарь флагов.
    """
    ...
    # Используем EDA-ядро из S03 м вычисляем флаги
    summary = summarize_dataset(df)
    missing_df = missing_table(df)
    flags_all = compute_quality_flags(summary, missing_df)
