...
- вызывает функции из `eda_cli.core`:
  ...
  - `missing_table`,
  - `compute_quality_flags`;
- возвращает булевы флаги из оценки датасета.
...
Ответ будет содержать флаги:

`"too_few_rows"` - флаг, показывающий, содержит ли датасет недостаточное количество строк
`"too_many_missing"` - флаг, показывающий, что количество пропущенных значений в датасете превышает порог
...
`"has_many_zero_values"` - флаг, если доля нулей в числовых колонках превышает выбранный порог.
