...
|----|---------------------------------------------------------------------------|------------------|-----------------------------------------------------|
| 1  | Сервис запускается по инструкциям из `project/README.md` и работает      |       Да         | `README.md`, разделы "Требования и установка", «Как запустить»       |
| 2  | Endpoint `/predict` использует **реальную модель**, а не заглушку        |       Да         | `src/service/`, `src/models/`            |
| 3  | Есть EDA и хотя бы один эксперимент с метриками                          |       Да         | `notebooks/01_eda.ipynb`, `report.md`     |
| 4  | Есть baseline и улучшенная модель, есть **сравнение по метрикам**        |       Да         | `notebooks/02_baseline_models_training.ipynb`, `notebooks/03_advanced_models.ipynb`, `report.md` |
| 5  | Код не свален в один ноутбук: есть внятная структура в `src/`            |       Да         | `src/data/`, `src/models/`, `src/service/` |
| 6  | Есть Dockerfile **или** понятный сценарий развёртывания без Docker       |       Да         | `Dockerfile`, шаги запуска описаны в `README.md`       |
| 7  | Есть `.env.example` и **нет** в репозитории реальных секретов/паролей    |       Да         | `.env.example`, `SECURITY.md`, `.gitignore` исключает `.env`    |
| 8  | Реализованы логи/наблюдаемость (хотя бы консольные логи + `/health`)     |       Да         | `src/service/middleware.py` — `ObservabilityMiddleware`, endpoints `/health` и `/metrics` |
| 9  | В `report.md` **обоснован выбор финальной модели** по результатам экспериментов |Да         | `report.md` в разделе 5 содержит таблица сравнения моделей и обоснование выбора BM25  |
| 10 | `project/README.md` и `report.md` позволяют понять сценарий демонстрации |       Да         | `README.md` раздел 7 «Демонстрация на защите», `report.md` раздел 9 — пошаговый сценарий|
